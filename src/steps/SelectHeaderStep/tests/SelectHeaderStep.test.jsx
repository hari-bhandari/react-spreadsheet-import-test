...
describe("Select header step tests", () => {
    test("Select header row and click next", async () => {
        ...
        render(<Providers theme={defaultTheme} rsiValues={mockRsiValues}>
    ...
    test("selectHeaderStepHook should be called after header is selected", async () => {
        ...
        await waitFor(() => {
            expect(selectHeaderStepHook).toBeCalledWith(["name", "age", "date"], [
                ["Josh", "2", "2020-03-03"],
    ...
    test("selectHeaderStepHook should be able to modify raw data", async () => {
        ...
        render(<ReactSpreadsheetImport {...mockRsiValues} selectHeaderStepHook={selectHeaderStepHook} initialStepState={{
    ...
    test.skip("trailing (not under a header) cells should be rendered in SelectHeaderStep table, " +
        "but not in MatchColumnStep if a shorter row is selected as a header", async () => {
        const selectHeaderStepHook = jest.fn(async (headerValues, data) => {
            return { headerValues, data };
        });
        render(<ReactSpreadsheetImport {...mockRsiValues} selectHeaderStepHook={selectHeaderStepHook}/>);
